...
import { LockSettingsModel, type ILockSettings } from "./lock-model";
...
export class LockRepository {
  ...
  static async getSettings(userId: string): Promise<ILockSettings | null> {
    ...
    await connectDB();
    const settings = await (LockSettingsModel as Model<ILockSettings>)
      .findOne({ userId })
      .lean();
    ...
    if (serializedSettings) {
      await setCachedValue(
        cacheKey,
        serializedSettings,
        LOCK_SETTINGS_CACHE_TTL_SECONDS,
      );
    }
  ...
  static async saveSettings(
    ...
    const settings = await (
      LockSettingsModel as Model<ILockSettings>
    ).findOneAndUpdate(
      { userId },
      {
        $set: data,
