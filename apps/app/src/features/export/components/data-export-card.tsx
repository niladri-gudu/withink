  ...
  const handleExport = async () => {
    ...
    try {
      if (isClientEncrypted && masterKey) {
        ...
        for (const entry of entries) {
          const contentHtml = await safeDecryptText(
            entry.contentHtml,
            masterKey,
          );
          const contentText = await safeDecryptText(
            entry.contentText,
            masterKey,
          );
          const decTitle = await safeDecryptText(entry.title, masterKey);
          ...
          monthFolder?.file(
            `${entry.date}.html`,
            buildEntryHtml(title, entry.date, contentHtml),
          );
        }
  ...
  return (
    <Card className="p-6 sm:p-8">
      <div className="flex items-start gap-4">
        <div className="border-border bg-secondary/60 text-muted-foreground mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border">
          <Archive className="h-5 w-5" />
        </div>
        <div className="space-y-1">
          <h2 className="text-h3">Your data</h2>
          <p className="text-body-small text-muted-foreground">
            Download a complete backup of your journal — plain text, formatted
            entries, and a metadata file.
          </p>
        </div>
      </div>
      ...
      <div className="mt-7 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
        <p className="text-body-small text-muted-foreground">
          Everything you&apos;ve written, packaged into a single ZIP you own
          forever.
        </p>
        <Button
