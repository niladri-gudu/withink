...
describe("ExportService.generateExportZip", () => {
  ...
  it("builds a full backup with metadata, txt, html, and images", async () => {
    const entryWithImage = makeEntry({
      ...
      contentJson: {
        ...
        content: [
          {
            type: "image",
            attrs: { src: `${PUBLIC_BASE}/${INTERNAL_IMAGE_KEY}` },
          },
        ],
      },
    });
    ...
    expect((r2.send as any).mock.calls[0][0].input.Key).toBe(
      INTERNAL_IMAGE_KEY,
    );
  });
