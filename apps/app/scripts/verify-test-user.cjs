...
async function main() {
  ...
  const lock = await db
    .collection("locksettings")
    .findOne({ userId: userIdStr });
  const entryCount = await db
    .collection("entries")
    .countDocuments({ userId: userIdStr });
  ...
  console.log(
    `user        : ${user.name} <${user.email}> verified=${user.emailVerified}`,
  );
  console.log(
    `id types    : user._id=${desc(user._id)} account._id=${desc(account._id)} account.userId=${desc(account.userId)} account.accountId=${desc(account.accountId)}`,
  );
  console.log(
    `login hash  : ${(await verifyScrypt(account.password, LOGIN_PASSWORD)) ? "OK (test@123)" : "FAIL"}`,
  );
  console.log(`entries     : ${entryCount}`);
  console.log(
    `lock enabled: ${lock.isLockEnabled} (passcodeHash present: ${!!lock.passcodeHash})`,
  );

  // Decrypt the master key with the diary password.
  const diaryKey = crypto.pbkdf2Sync(
    DIARY_PASSWORD,
    Buffer.from(enc.encryptionSalt, "hex"),
    100000,
    32,
    "sha256",
  );
  ...
  console.log(
    `master key  : decrypts with diary password -> ${masterKeyHex === masterKeyHex.slice(0, 100) ? "yes" : "??"} (${masterKeyHex.length} hex chars)`,
  );
  ...
  for (const e of sampleEntries) {
    ...
    console.log(
      `  ${e.date} | mood=${e.mood} | words=${words} | "${title.slice(0, 40)}..." | plaintext=${!title.includes(":") && text.length > 50}`,
    );
  }
