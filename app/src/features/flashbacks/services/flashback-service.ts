...
export class FlashbackService {
  ...
  static async getFlashbackForToday(
    ...
    const cached = await getCachedValue<{ entryDate: string; label: string }>(cacheKey);
    if (cached) {
      const entry = await JournalService.getEntryForDate(userId, cached.entryDate, todayStr);
      if (entry) {
    ...
    const entry = await JournalService.getEntryForDate(userId, selectedDate, todayStr);
    return { entry, label };
  }
