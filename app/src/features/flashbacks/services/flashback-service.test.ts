...
describe("FlashbackService", () => {
  ...
  it("should return null if user has no entries", async () => {
    ...
    const result = await FlashbackService.getFlashbackForToday(mockUserId, mockToday);
  ...
  it("should return null if entries are only today or yesterday", async () => {
    ...
    vi.mocked(JournalService.getEntryDates).mockResolvedValue(["2026-07-02", "2026-07-01"]);
  ...
  it("should return cached flashback on subsequent calls", async () => {
    ...
    vi.mocked(getCachedValue).mockImplementation(async (key) => {
      if (key.includes("flashback:2026-07-02")) {
        return { entryDate: "2026-05-15", label: "A reflection from your archives" };
      }
      return null;
    });
  ...
  it("should invalidate cache and select a new flashback on refresh", async () => {
    ...
    const result = await FlashbackService.refreshFlashback(mockUserId, mockToday);

    expect(redis!.del).toHaveBeenCalledWith(expect.stringContaining("flashback:2026-07-02"));
    expect(result).not.toBeNull();
