...
describe("Lock Actions Suite", () => {
  ...
  describe("getLockSettingsAction", () => {
    ...
    it("should return lock settings and unlock status on success", async () => {
      ...
      vi.mocked(cookieStore.get).mockReturnValue({ value: "valid-cookie" } as any);
  ...
  describe("unlockAction", () => {
    ...
    it("should set unlock cookie and return success on correct passcode", async () => {
      ...
      const spyVerify = vi.spyOn(LockService, "verifyPasscode").mockReturnValue(true);
      const spyCookie = vi.spyOn(LockService, "setUnlockCookie").mockResolvedValue(undefined);
    ...
    it("should return error on incorrect passcode", async () => {
      ...
      const spyVerify = vi.spyOn(LockService, "verifyPasscode").mockReturnValue(false);
  ...
  describe("lockAction", () => {
    it("should clear the unlock cookie and return success", async () => {
      const spyClear = vi.spyOn(LockService, "clearUnlockCookie").mockResolvedValue(undefined);
  ...
  describe("saveLockSettingsAction", () => {
    it("should save settings and set unlock cookie when enabling", async () => {
      ...
      const spyHash = vi.spyOn(LockService, "hashPasscode").mockReturnValue("salt:hash");
      const spyCookie = vi.spyOn(LockService, "setUnlockCookie").mockResolvedValue(undefined);
  ...
  describe("verifyPasswordAndResetLockAction", () => {
    it("should verify password and disable lock on success", async () => {
      ...
      const spyVerifyPass = vi.spyOn(LockService, "verifyLoginPassword").mockResolvedValue(true);
      const spyClear = vi.spyOn(LockService, "clearUnlockCookie").mockResolvedValue(undefined);

      const res = await verifyPasswordAndResetLockAction("login-password-123");
      expect(res.success).toBe(true);
      expect(spyVerifyPass).toHaveBeenCalledWith("writer@withink.me", "login-password-123");
      expect(LockRepository.saveSettings).toHaveBeenCalledWith(mockUserId, {
        isLockEnabled: false,
        passcodeHash: "",
      });
    ...
    it("should fail and not disable lock on incorrect password", async () => {
      ...
      const spyVerifyPass = vi.spyOn(LockService, "verifyLoginPassword").mockResolvedValue(false);
