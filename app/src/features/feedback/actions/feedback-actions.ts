...
import { feedbackSchema, type FeedbackInput } from "../validation/feedback-schema";
...
export async function submitFeedbackAction(
  ...
  try {
    ...
    if (!parsed.success) {
      const first = parsed.error.issues[0]?.message ?? "Please check your submission.";
    ...
    if (imageUrl && !isOwnedFeedbackImage(imageUrl, userId)) {
      return { success: false, error: "That attachment could not be verified." };
    }
