...
const okResult = { success: true, data: { updatedAt: "2026-01-01T00:00:00.000Z" } };
...
describe("useAutoSave", () => {
  ...
  it("saves after the debounce delay when content changes", async () => {
    ...
    expect(saveEntryActionMock).toHaveBeenCalledWith(
      expect.objectContaining({ date: "2026-01-01", title: "Hello world", mood: null }),
      expect.any(String),
    );
  ...
  it("serializes saves and persists the newest content typed during a save", async () => {
    ...
    expect(saveEntryActionMock).toHaveBeenCalledTimes(1);
    expect(saveEntryActionMock).toHaveBeenCalledWith(expect.objectContaining({ title: "Draft 1" }), expect.any(String));
    ...
    expect(saveEntryActionMock).toHaveBeenLastCalledWith(expect.objectContaining({ title: "Draft 2" }), expect.any(String));
  });
  ...
  it("shows the locked status when the server reports a locked session", async () => {
    ...
    expect(saveEntryActionMock).toHaveBeenCalledTimes(2);
    expect(saveEntryActionMock).toHaveBeenLastCalledWith(expect.objectContaining({ title: "Unlocked draft" }), expect.any(String));
    expect(result.current).toBe("saved");
  });
  ...
  it("flushes pending changes on pagehide without waiting for the debounce", async () => {
    ...
    expect(saveEntryActionMock).toHaveBeenCalledTimes(1);
    expect(saveEntryActionMock).toHaveBeenCalledWith(expect.objectContaining({ title: "Before hide" }), expect.any(String));
  });
  ...
  it("resets the baseline when the date changes", async () => {
    ...
    rerender({ data: makeData({ date: "2026-01-02", title: "New day" }), enabled: true });
    await act(async () => {
      await vi.advanceTimersByTimeAsync(1500);
    });
    expect(saveEntryActionMock).toHaveBeenCalledTimes(1);
