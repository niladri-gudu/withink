...
function loadEnv() {
  ...
  return {
    mongodbUri,
    redisUrl: process.env.UPSTASH_REDIS_REST_URL || dotEnv.UPSTASH_REDIS_REST_URL,
    redisToken: process.env.UPSTASH_REDIS_REST_TOKEN || dotEnv.UPSTASH_REDIS_REST_TOKEN,
  };
}

async function purgeRedis(redisUrl, redisToken, dbName) {
  if (!redisUrl || !redisToken) {
    console.log("  (skipping Redis purge: no UPSTASH_REDIS_REST_URL/TOKEN available)");
    return 0;
  }
  ...
  for (let page = 0; page < 50; page++) {
    const scanRes = await fetch(`${base}/scan/${cursor}?match=lock:*:settings&count=1000`, {
      headers,
    });
    if (!scanRes.ok) {
  ...
  console.log(`  Redis: purged ${deleted} lock settings cache key(s) (${dbName})`);
  return deleted;
}
...
async function main() {
  ...
  const before = await col.countDocuments({ passcodeHash: { $ne: "", $exists: true } });
  console.log(`Before: ${before} document(s) have a passcodeHash set`);

  if (hasFlag("--dry-run")) {
    console.log("Dry run only — no changes were made. Re-run without --dry-run to apply.");
  ...
  const after = await col.countDocuments({ passcodeHash: { $ne: "", $exists: true } });
